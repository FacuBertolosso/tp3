...
public class InitErrorHandler : MonoBehaviour {
	...
	private void OnInitError(VuforiaUnity.InitError error) {
		if (error != VuforiaUnity.InitError.INIT_SUCCESS) {
			// BEGIN: Disable Cardboard stereo mode before showing the init error message.
			// NOTE: Choose the lines appropriate to your version of Cardboard/VR SDK:
			Cardboard.SDK.VRModeEnabled = false; // Cardboard SDK 0.5.x, 0.6.0
